...
const Navbar2 = () => {
  ...
  return (
    <main>
    <nav className="flex justify-between py-4 px-10 items-center">
      <div className="flex justify-center items-center gap-4">
        <GiHamburgerMenu className="text-3xl text-gray-900 cursor-pointer"/>
        <img src={Logo} alt="" className="w-8 h-8 cursor-pointer"/>
      </div>

      <section className="w-screen h-full fixed bg-black/50 top-0 right-0 backdrop-blur-sm lg:hidden">
        ...
        {navList.map(({id,title})=> {
          return <h3 key={id} className="cursor-pointer text-2xl font-bold">{title}</h3>
        })}
        </div>
      </section>
